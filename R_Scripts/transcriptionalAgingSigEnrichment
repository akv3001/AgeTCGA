...
breast_sig <- list(aging_up = agingup, aging_down = agingdown)
...
normal_ssgsea <- data.frame(normal_ssgsea[, c(1,2,3,37,38)])
...
cancer_ssgsea <- data.frame(cancer_ssgsea[, c(1,2,3,37,38)])
...
metabric_clinical <- read.csv("./DATA/brca_metabric_all_Data/data_clinical_patient.txt",
...
metabric_expression <- read.csv("./DATA/brca_metabric_all_Data/data_expression.txt", 
                                sep = "\t", header = T, stringsAsFactors = F)
...
colnames(metabric_gsea)[5] <- "Quartiles"
...
# Load GTEx data
###########################################################################################################
meta1 <- read.csv("/Users/Yajas/Documents/Elemento/AgeTCGA-master/DATA/GTEx/GTEx_Analysis_v8_Annotations_SampleAttributesDS.txt",
...
meta2 <- read.csv("/Users/Yajas/Documents/Elemento/AgeTCGA-master/DATA/GTEx/GTEx_Analysis_v8_Annotations_SubjectPhenotypesDS.txt",
                  stringsAsFactors = F, header = T, sep = "\t")
gtex_data <- read.table("/Users/Yajas/Documents/Elemento/AgeTCGA-master/DATA/GTEx/Tissues/GTEx_Breast - Mammary Tissue.txt")
...
colnames(gtex_ssgsea)[5] <- "Quartiles"
...
metabric_gsea <- metabric_gsea[,-6]
combinedDF <- data.frame(rbind(gtex_ssgsea, normal_ssgsea, cancer_ssgsea, metabric_gsea))
...
combinedDF$Dataset <- factor(combinedDF$Dataset, levels = c("GTEx Breast", "TCGA-BRCA Normal", "TCGA-BRCA Cancer", "METABRIC"))
combinedDF$Type <- ifelse(as.character(combinedDF$Dataset) == "GTEx Breast", "Healthy",
                          ifelse(as.character(combinedDF$Dataset) == "TCGA-BRCA Normal", "Healthy", "Cancer"))
combinedDF$Type <- factor(combinedDF$Type, levels = c("Healthy", "Cancer"))

# PLOT
###########################################################################################################
ggboxplot(combinedDF, x = 'Dataset', y = 'aging_up', fill = 'Dataset', palette = "aaas", facet.by = 'Quartiles',
          xlab = "", ylab = "Enrichment Score", title = "Transcriptional Aging Signature", scales = "free",
          subtitle = "Paired Breast Cancer Cases") + 
  font(object = "title", size = 20) + 
  font(object = "y", size = 20) +
  font(object = "subtitle", size = 15) +
  font(object = "legend.title", size = 20) +
  font(object = "legend.text", size = 20) +
  font(object = "xy.text", size = 20)+
...
stat.test <- combinedDF %>%
  group_by(Type) %>%
  group_by(Dataset) %>%
  wilcox_test(aging_up ~ Quartiles)
stat.test$stars <- stars.pval(stat.test$p)


ggboxplot(combinedDF, x = 'Quartiles', y = 'aging_up', fill = 'Quartiles', palette = "jco", 
          xlab = "", ylab = "ssGSEA Enrichment Score", title = "Transcriptional Age Acceleration", scales = "free") + 
  facet_wrap(facets = ~Type + Dataset, scales = "free_y", nrow = 1) +
  theme_pubr(base_size = 35) +
  labs(fill = "Age") +
  theme(legend.position = "none",
        strip.text = element_text(size = 25)) +
  stat_pvalue_manual(data = stat.test, label = "stars", y.position = c(1,1,1,1))
  # stat_compare_means(method = "wilcox", label = "p.signif", size = 10, label.x = c(1.5, 1.42, 1.5, 0.9))
ggsave("./Final/Figures/Figure 2/transcriptional_age_acc.jpg", dpi = 320, width = 20, height = 9)
